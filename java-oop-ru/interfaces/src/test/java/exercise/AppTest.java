...
class AppTest {
    ...
    void testReversedSequence() {
        String text = "qwerty";
        String reversed = "ytrewq";

        var result = new ReversedSequence(text).toString();
        assertThat(result).isEqualTo(reversed);

        assertThat(result.length()).isEqualTo(text.length());

        assertThat(result.charAt(2)).isEqualTo('r');
        assertThat(result.charAt(4)).isEqualTo('w');

        assertThat(result.subSequence(1,4)).isEqualTo("tre");
    }
    // END
}
